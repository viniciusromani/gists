//
//  GistDetailsViewModelTest.swift
//  GithubGistsTests
//
...
class GistDetailsViewModelTest: XCTestCase {
    ...
    func testInvalidAttributes() {
        model = Gist(id: "3aacd6b893f35c2decdd8bbae9a37a3d",
                     ...
                     isPublic: nil,
                     createdAt: nil)
        let gistDetailsViewModel = GistDetailsViewModel(mapping: model!)
        
        expect(gistDetailsViewModel.name).to(beEmpty())
        expect(gistDetailsViewModel.description).to(equal("-"))
        expect(gistDetailsViewModel.url).to(equal("-"))
        expect(gistDetailsViewModel.language).to(equal("-"))
        expect(gistDetailsViewModel.forks).to(equal("1"))
        expect(gistDetailsViewModel.ownerName).to(equal("-"))
        expect(gistDetailsViewModel.ownerGithubURL).to(equal("-"))
    }
    
    /*
     *  Parameters we want to cover:
     *  - name should be equal to the first item of files array
     *  - language should be equal to the first item of files array
     */
    func testTwoFilesArray() {
        ...
        expect(gistDetailsViewModel.name).to(equal(model?.files.first?.name))
        expect(gistDetailsViewModel.language).to(equal(model?.files.first?.language))
    }
}

extension GistDetailsViewModelTest: TestableViewModel {
    ...
    func testSuccessFullMapping() {
        ...
        expect(gistDetailsViewModel.name).to(equal(model?.files.first?.name))
        expect(gistDetailsViewModel.description).to(equal(model?.description))
        expect(gistDetailsViewModel.url).to(equal(model?.htmlURL?.absoluteString))
        expect(gistDetailsViewModel.language).to(equal(model?.files.first?.language))
        expect(gistDetailsViewModel.forks).to(equal("1"))
        expect(gistDetailsViewModel.ownerName).to(equal(model?.owner?.userName))
        expect(gistDetailsViewModel.ownerGithubURL).to(equal(model?.owner?.userGithubURL?.absoluteString))
    }
}
